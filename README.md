...
A chave de API padrão está configurada no arquivo:
👉 [src/services/gemini.ts](file:///c:/Users/pedro/Downloads/CaloriQ%203.0/src/services/gemini.ts)

Se você desejar utilizar a sua própria chave obtida no **Google AI Studio**:
1. Acesse o arquivo [gemini.ts](file:///c:/Users/pedro/Downloads/CaloriQ%203.0/src/services/gemini.ts).
2. Substitua o valor da constante `GEMINI_API_KEY`:
   ```typescript
